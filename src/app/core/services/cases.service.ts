...
export class CasesService {
  ...
  /**
   * جلب القضايا. لو بعتّ clientId هيتضاف كـ query param (client=...)
   * علشان لو الـ backend عنده دعم للفلترة بالعميل، تقدر تستخدمها مباشرة
   * بدل الفلترة في الفرونت.
   */
  getAll(page = 1, limit = 10, status?: string, search?: string, clientId?: string) {
    let params = new HttpParams().set('page', page).set('limit', limit);
    if (status) params = params.set('status', status);
    if (search) params = params.set('q', search);
