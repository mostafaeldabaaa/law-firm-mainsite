...
//   {
//     // الصفحة الرئيسية العامة — بتظهر لأي حد (مسجل دخول أو لأ)، ومفيش
//     // عليها أي guard. لو المستخدم داخل حسابه بالفعل، الكومبوننت نفسه
//     // بيحوّله على طول للوحة التحكم (شوف home.component.ts).
//     path: '',
//     pathMatch: 'full',
...
export const routes: Routes = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'sessions',          loadComponent: () => import('./features/sessions/session-list/session-list.component').then(m => m.SessionListComponent) },
      { path: 'sessions/new',      loadComponent: () => import('./features/sessions/session-form/session-form.component').then(m => m.SessionFormComponent) },
      // مهم: 'clients/new' لازم يجي قبل 'clients/:id' في الترتيب، وإلا
      // Angular هيطابق كلمة "new" على إنها قيمة الـ :id بالغلط (زي ما
      // كان بيحصل بالظبط قبل الإصلاح ده).
      {
        path: 'clients',
        canActivate: [adminGuard],
        children: [
          { path: '',       loadComponent: () => import('./features/clients/client-list/client-list.component').then(m => m.ClientListComponent) },
          { path: 'new',    loadComponent: () => import('./features/clients/client-form/client-form.component').then(m => m.ClientFormComponent) },
          { path: ':id',    loadComponent: () => import('./features/clients/client-detail/client-detail.component').then(m => m.ClientDetailComponent) },
          { path: ':id/edit', loadComponent: () => import('./features/clients/client-form/client-form.component').then(m => m.ClientFormComponent) },
        ],
      },
      { path: 'lawyers',           loadComponent: () => import('./features/lawyers/lawyer-list/lawyer-list.component').then(m => m.LawyerListComponent) },
      { path: 'lawyers/:id',       loadComponent: () => import('./features/lawyers/lawyer-detail/lawyer-detail.component').then(m => m.LawyerDetailComponent) },
