...
@Component({
  ...
  template: `
    <div class="sl-page" dir="rtl">
      ...
      <!-- ===== Skeleton Loading: Desktop/Tablet Table ===== -->
      <div class="sl-table-wrap sl-desktop-only" *ngIf="loading">
      ...
      <!-- ===== Skeleton Loading: Mobile Cards ===== -->
      <div class="sl-mobile-only" *ngIf="loading">
        <div class="sl-mcard" *ngFor="let i of skeletonRows">
          <div class="sl-sk sl-sk-w120"></div>
          <div class="sl-sk sl-sk-w80" style="margin-top:8px"></div>
          <div class="sl-sk sl-sk-pill sl-sk-w60" style="margin-top:8px"></div>
        </div>
      </div>
      ...
      <div class="sl-table-wrap sl-desktop-only" *ngIf="!loading && sessions.length > 0">
        <table class="sl-table">
          <thead>
            <tr>
              <th>التاريخ والوقت</th>
      ...
      <!-- ===== Mobile Cards ===== -->
      <div class="sl-mobile-only" *ngIf="!loading && sessions.length > 0">
        <div class="sl-mcard" *ngFor="let s of sessions" [id]="'session-row-' + s._id" [class.sl-highlight]="highlightedSessionId === s._id">

          <div class="sl-mcard-top">
            <div>
              <div class="sl-date-main">{{ s.startTime | date:'EEE، d MMM y' }}</div>
              <div class="sl-date-sub">{{ s.startTime | date:'HH:mm' }} - {{ s.endTime | date:'HH:mm' }}</div>
            </div>
          ...
          <div class="sl-mcard-case">
            <div class="sl-case-title">{{ s.case?.title }}</div>
            <div class="sl-case-num">{{ s.case?.caseNumber }}</div>
          </div>

          <div class="sl-mcard-info">
            <span class="sl-badge" [ngClass]="'sl-type-' + s.type">{{ typeLabel(s.type) }}</span>
            <span *ngIf="s.location" class="sl-location-value">
              <svg viewBox="0 0 24 24" class="sl-icon sl-icon-sm"><path d="M12 21s-7-6.2-7-11a7 7 0 1 1 14 0c0 4.8-7 11-7 11z"/><circle cx="12" cy="10" r="2.3"/></svg>
              {{ s.location }}
          ...
          <div *ngIf="s.notes" class="sl-mcard-notes">{{ s.notes }}</div>
  ...
  styles: [`
    ...
    .sl-location-value { display: inline-flex; align-items: center; gap: 5px; font-size: 12px; color: #64748b; }
    ...
    @media (max-width: 1024px) {
      .sl-page { padding: 18px; }
      .sl-case-title, .sl-location, .sl-notes { max-width: 120px; }
    }

    /* Mobile: أقل من 768px */
    @media (max-width: 768px) {
      .sl-page { padding: 14px; }

      .sl-header {
        flex-direction: column;
        align-items: stretch;
      }
      .sl-btn-primary { width: 100%; }

      .sl-form-grid  { grid-template-columns: 1fr; }
      .sl-field--full { grid-column: 1; }

      .sl-filters { flex-wrap: wrap; }
      .sl-select-wrap { min-width: 0; }

      /* التبديل من الجدول للكاردز */
      .sl-desktop-only { display: none !important; }
      .sl-mobile-only { display: block; }

      .sl-modal { margin: 0; max-width: 100%; padding: 18px; }
      .sl-details-row { flex-direction: column; gap: 4px; }
      .sl-details-value { text-align: right; }

      .sl-form-actions { flex-direction: column-reverse; }
    }

    @media (max-width: 480px) {
      .sl-title { font-size: 18px; }
      .sl-mcard-actions .sl-icon-btn span { display: none; }
    }
  `]
})
